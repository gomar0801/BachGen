...
def measures_to_tokens(measures, soup, staff=None, note_name=True):
    ...
    for measure in measures:
        ...
        if not voices:
            for n in notes:
                # On crée artificiellement un tag <voice>1</voice> si absent
                if not n.voice:
                    new_voice_tag = soup.new_tag("voice")
                    new_voice_tag.string = "1"
                    n.append(new_voice_tag)
            voices = ["1"]  # On force la voix 1
        #######################  
...
def MusicXML_to_tokens_accords_sans_voice(soup_or_mxml_path, note_name=True): # use this method
