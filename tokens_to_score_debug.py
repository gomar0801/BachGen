...
def note_token_to_obj(tokens, key):
    print("\n[Conversion note/rest]")
    print("Tokens :", tokens)

    if tokens[0] == 'rest': # for rests
        print("→ C’est un silence.")
...
def tokens_to_PartStaff(tokens, key_=0, start_voice=1):
    print("\n=== Début de tokens_to_PartStaff ===")
    print("Tokens initiaux :", tokens)
    tokens = concatenated_to_regular(tokens)
    print("→ Après concatenated_to_regular :", tokens)
    ...
    print("→ Après aggr_note_token :", tokens)

    for i, t in enumerate(tokens):
        print(f"\n[Token {i}] {t}")
        if t == 'bar':
            print("→ Nouvelle mesure (bar)")
            if i != 0:
                p.append(m)
            m = stream.Measure()
            voice_id = start_voice
            ...
            print(f"→ Note ou accord détecté : {t.split()}")
            n = note_token_to_obj(t.split(), k)
            print("→ Objet music21 généré :", n)
            if ottava_flag:
    ...
    print("=== Fin de tokens_to_PartStaff ===\n")
    return p
